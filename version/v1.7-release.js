...
(function() {
            ...
            try {
                if (this._requestUrl && this._requestUrl.includes("/queryStuPaper/v2")) {
                    ...
                    if (this.responseType === 'blob' && this.response) {
                        this.response.text().then(text => processPaperData(JSON.parse(text)));
                    } else if (this.responseType === 'json' && this.response) {
                        processPaperData(this.response);
                    } else if ((this.responseType === '' || this.responseType === 'text') && this.responseText) {
                        processPaperData(JSON.parse(this.responseText));
                    }
                }
            } catch (e) {
    ...
    function processPaperData(jsonData) {
        ...
        let questionsArray = jsonData.data.questions;
        globalQuestionsData =[];
        globalImageAssets = [];
        globalPdfQuestions = [];

        let resultText = `【小雅粘粘粘：AI 作答模板】
...
`;
        ...
        questionsArray.forEach((q, index) => {
            ...
            let qTitle = parsedTitle.text;
            let qTypeStr = getQuestionTypeLabel(q.type);
            const resultOptions = [];
            const matchingLeftItems = [];
            const matchingRightItems = [];
            collectImageAssets(questionIndex, 'title', null, parsedTitle);
            
            resultText += `${questionIndex}. ${qTitle} ${qTypeStr}\n`;

            let sortedItems = Array.isArray(q.answer_items) ? [...q.answer_items] : [];
            if (q.answer_items_sort && Array.isArray(q.answer_items)) {
                const sortIds = String(q.answer_items_sort).split(',');
                sortedItems = sortIds.map(id => q.answer_items.find(item => String(item.id) === String(id))).filter(Boolean);
            }
            ...
            const pdfQuestion = {
                ...
                type: q.type,
                typeLabel: qTypeStr,
                titleSegments: parsedTitle.segments && parsedTitle.segments.length ? parsedTitle.segments : (qTitle ? [{ type: 'text', value: qTitle }] : []),
                options: [],
                matchingLeftItems: [],
                matchingRightItems: [],
            ...
            if (q.type === 1 || q.type === 2 || q.type === 5) {
                ...
                sortedItems.forEach(opt => {
                    ...
                    pdfQuestion.options.push({
                        letter: optionLetter,
                        text: optText,
                        segments: parsedOption.segments && parsedOption.segments.length ? parsedOption.segments : (optText ? [{ type: 'text', value: optText }] : [])
                    });
                    resultOptions.push({
                        id: opt.id,
                        letter: optionLetter,
                    ...
                    resultText += `   ${optionLetter}. ${optText}\n`;
                    letterCharCode++;
                });
            } else if (q.type === 4) {
                resultText += `   (本题共 ${sortedItems.length} 个填空)\n`;
            } else if (q.type === 7) {
                resultText += `   附件题无需回答。\n`;
            } else if (q.type === 13) {
                ...
                leftRawItems.forEach((opt, itemIndex) => {
                    ...
                    const segments = parsedOption.segments && parsedOption.segments.length ? parsedOption.segments : (optText ? [{ type: 'text', value: optText }] : []);
                    collectImageAssets(questionIndex, 'option', optionLetter, parsedOption);
                    const itemData = {
                        id: opt.id,
                        letter: optionLetter,
            ...
            globalPdfQuestions.push(pdfQuestion);
        });

        globalExtractedText = resultText;
        ...
        resultFilter = 'all';
        imageDrawerVisible = false;
        console.log("✅ 数据清洗完毕，已生成终极Prompt！");
        createUIPanel();
    }
    ...
    function renderPrintQuestion(question, imageMap) {
        let detailHtml = '';
        ...
        return `
            <section class="xy-print-question">
                <h2>${escapeHTML(question.index)}. ${escapeHTML(question.typeLabel)}</h2>
    ...
    function buildPrintDocumentHtml(pdfQuestions, imageMap, title) {
        const questionsHtml = pdfQuestions.length
            ? pdfQuestions.map(question => renderPrintQuestion(question, imageMap)).join('')
...
<body>
    ...
    function writePrintWindowLoading(printWindow, title) {
        ...
        printWindow.document.write(`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><title>${escapeHTML(title)}</title></head><body style="margin:0;padding:32px;font:15px/1.7 Arial,'Microsoft YaHei',sans-serif;color:#374151;"><strong>正在生成 AI 图文 PDF...</strong><div style="margin-top:8px;color:#6b7280;">正在下载题目图片，请稍候。</div></body></html>`);
    ...
    async function exportPaperAsPdf() {
        ...
        try {
            ...
            printWindow.document.open();
            printWindow.document.write(buildPrintDocumentHtml(globalPdfQuestions, imageMap, fileName));
            printWindow.document.close();
    ...
    function injectUIStyles() {
        if (document.getElementById('xy-v17-style')) return;
        const style = document.createElement('style');
        style.id = 'xy-v17-style';
        style.textContent = `
            #xy-magic-box {
                --xy-bg: #f7f8f8;
            ...
            .xy-main-scroll, .xy-result-scroll, .xy-image-drawer {
            ...
            .xy-image-drawer.xy-drawer-open {
                opacity: 1;
                pointer-events: auto;
            ...
            @media (prefers-reduced-motion: reduce) {
                ...
                #xy-panel-shell, #xy-result-panel, .xy-image-drawer, #xy-floating-trigger, .xy-action-btn, .xy-submit-btn, .xy-icon-btn, .xy-mini-btn, .xy-notice-action, .xy-result-tab {
    ...
    function renderSubmissionResultPanel() {
        ...
        summary.innerHTML = `
            <div class="xy-score-block">
                ...
                <div class="xy-score-detail">${escapeHTML(result.correctNum ?? '-')} 正确 · ${escapeHTML(wrongCount)} 错误 · ${escapeHTML(pendingCount)} 待批改</div>
            </div>
            <div class="xy-result-tabs">
                ...
                <button class="xy-result-tab ${resultFilter === 'bad' ? 'xy-active' : ''}" type="button" data-filter="bad">错题 ${escapeHTML(wrongCount)}</button>
        ...
        const filteredResults = questionResults.filter(item => resultFilter === 'all' || item.tone === resultFilter);
